...
> git是一个分布式版本控制系统，与集中的版本控制系统相比，在使用git的时候本地也会被当做一个版本库,Git没有网络也可以工作，而SVN工作的时候必须要联网  
...
# Git 是分布式版本控制系统，所以，每个用户都应该有自己识别ID：username和email
...
######- 小结
>- 当修改了了工作区某个文件的内容，想直接丢弃工作区的修改时，用命令`git restore <filename>`。
>- 当你不但改乱了工作区某个文件的内容，还添加到了暂存区时，想丢弃修改，分两步，第一步用命令`git restore --staged <filename>`，就回到了场景1，第二步按场景1操作。
>- 已经提交了不合适的修改到版本库时，想要撤销本次提交，可通过版本回退解决，不过前提是没有推送到远程库。
...
> Git 的版本库里存了很多东西，其中最重要的就是称为 stage（或者叫index）的暂存区，还有 Git 为我们自动创建的第一个分支 master，以及指向 master 的一个指针叫 HEAD；提交文件的过程使用git add将文件提交到暂存区，然后git commit命令将文件提交到分支master。 

#####  五、远程仓库 :
###### 1. 关联远程仓库 (已经有了远程仓库和本地仓库)
```
$ git remote add origin https://github.com/LHaisong/learn.git
```
...
$ git remote add github https://github.com/LHaisong/learn.git
# 2. 再关联Gitee的远程库：(注意:远程库的名称叫 gitee，不叫 origin)
$ git remote add gitee <name>
# 3. 推送到远程库
...
###### 3. 从远程仓库克隆 (先有远程库)
```
$ git clone https://github.com/LHaisong/learn.git
